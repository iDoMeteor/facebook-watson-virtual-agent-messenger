...
function receivedFromFacebook(message, senderID) {
  ...
  console.log('DEBUG:');
  console.log('Bot ID:', WATSON_BOT_ID);
  console.log('Watson Key:', WATSON_KEY);
  console.log('Watson Secret:', WATSON_SECRET);
  receivedFromWatson({
    message: {
      text: ['This is a hardcoded debug message'],
    }
  }, senderID);

  // Forumlate insert object
  const obj = {
    message: message,
    senderID: senderID,
    type: 'inquiry',
  ...
  // Configure Watson interface
  watson.configure({
    XIBMClientID: WATSON_KEY,
    XIBMClientSecret: WATSON_SECRET
  })
  // THIS FAILS with 401 not authorized, even though credentials are valid
  .start(WATSON_BOT_ID)
  .then(response => {
    ...
    Conversations
      ...
      .toArray((error, result) => {
        if (result.length) {
          ...
          console.log('Attempting to resume dialog with id: ' + dialogID);
          dialogID = ('undefined' == typeof result[0].dialogID) ? null : result[0].dialogID;
        } else {
          // Use new ID
          console.log('Starting new dialog with id: ' + dialogID);
          dialogID = res.chatID;
        }
        // Send to Watson
        watson.send(WATSON_BOT_ID, dialogID, message)
          .then(res => {
            // Send response from Watson back to Messenger
            receivedFromWatson(res, senderID, dialogID);
          })
        .catch(err => {
          console.error('Error sending to Watson:');
          console.error(err);
        });
    });
  ...
  .catch(err => {
    console.error('Error starting to Watson:');
    console.error(err);
  });
...
function receivedFromWatson(response, senderID, dialogID) {
